...
```bash
curl -fsSL https://github.com/the-agentic-world/megara/releases/latest/download/install.sh | MEGARA_VERSION=v1.0.10 MEGARA_INSTALL_DIR="$HOME/.local/bin" sh
```

설치 스크립트는 macOS arm64를 지원하며 기본 설치 위치는 `/usr/local/bin`입니다.
...
프로젝트 범위 설치에서는 스킬 래퍼가 `.agents/skills/insane-search`에 있고, 도구 파일은 `.agents/tools/insane-search`, 실행 wrapper는 `.agents/bin/insane-search`입니다. 첫 실행 시 wrapper가 `.agents/state/tools/insane-search/venv`에 필요한 Python dependency를 자동 bootstrap합니다. 이 스킬은 상시 활성 스킬이 아니므로 단순 검색에는 개입하지 않습니다.
