...
```bash
curl -fsSL https://github.com/the-agentic-world/megara/releases/latest/download/install.sh | MEGARA_VERSION=v1.0.7 MEGARA_INSTALL_DIR="$HOME/.local/bin" sh
```
...
프로젝트 범위 설치에서는 도구 파일이 `.agents/tools/insane-search`에 있고, 실행 wrapper는 `.agents/bin/insane-search`입니다. 이 도구는 상시 활성 스킬이 아니므로 단순 검색에는 개입하지 않습니다.
